...
- **Exemplars de noticing** · `resources/canon/exemplars/{close,angle,diagnostic}.md` · sharp vs mushy + ce que l'expert a vu de non-évident + l'out honnête · montrer bat réciter (le levier le plus fort pour la posture et le jugement)
- **Matrice de durabilité** · `resources/canon/model-provider-matrix.json` · SSOT alias logique vers modèle concret + chaîne de repli · au swap de modèle on édite 1 fichier pas 85. Validation des alias au build (`build-manifest --strict`) · respecte la décision v2.46 no-runtime-check
- **Audit du substrat** · `resources/scripts/audit-substrate.py` + cible Makefile · le garde anti-context-rot de notre plus gros pari · flague le stale (timestamp au-delà du `ttl_days` / `velocity_tier` canon static-slow-drift-weekly-live) et les refs spectrum dangling · advisory, jamais bloquant

### Changed
- **Le close réconcilié** · conflit de doctrines levé (investigation-posture « close = question macro » vs master « affirme le move » · le master gagne, règle de conflit). Le close passe à **faits PUIS lecture experte PUIS mouvement proactif** · montrer les faits est BON (surtout post-scrape), la météo c'est de s'y arrêter sans interpréter puis rendre une question passive. L'**out honnête** (la data ne porte pas de move, voici l'unique inconnu bloquant + le chemin pour le lever) est un close légitime. La lecture se **PRODUIT en raisonnant sur le substrat** via la chaîne de diagnostic, pas une injonction d'interpréter. Sur `CLAUDE.md` racine + `contextual-intelligence.md` + `investigation-posture.md`
- **6 skills producteurs propagés et refoundés** · `snapshot-brand`, `profile-audience`, `produce-paid-angles`, `brief-day`, `build-atlas-complete`, `onboard-brand` · la prose prescriptive (« toujours close macro UNE question », l'inverse de la doctrine) remplacée par l'invocation du scaffold-sur-substrat + l'out + le pointeur exemplar · prose trimmée

### Notes
- **L'auto-critique en vol** (le missing middle) · définie comme passe nommée dans le master · avant de surfacer, le producteur relit son draft contre l'exemplar (EXTRINSÈQUE · juger le standard pas sa propre justif · boucle de réécriture) · attaque la météo AU RUNTIME où la barrière de release ne peut pas
- **La méthode** · validée par le champ comme doctrine first-party Anthropic/OpenAI/Google · faire raisonner le modèle SUR le substrat (scaffold) + montrer le NOTICING + un OUT honnête + une EVAL calibrée · la prose qui prescrit est le levier le plus faible, sur-prescrire un modèle frontière le fait régresser vers un script. 4 workflows multi-agents (propagation + refound + recherche terrain 7-angles + revue adverse)
- **La boucle bouclée** · la revue adverse qu'on a construite a attrapé un VRAI bug dans le travail qui la construisait · `audit-substrate` lisait un mauvais vocabulaire de fraîcheur (deviné sur un `_EXAMPLE` vide, `extracted_at` omis) · corrigé et re-vérifié
- **Gates** · validate-all --strict 0/0/0/0 · reachability CRITICAL/HIGH 0/0 (7 WARN dette advisory pré-existante) · eval 6/6 · alignment 1.0 (seed-only, honnête) · em-dash 0
- **Différé** · Phase 2/3 (câbler les validateurs en SubagentStop scopé + dégraissage + couche commandes + registre de format) · les nice-to-have non-bloquants de la revue (dédup calibrate/alignment, golden hollow runner-asserté, clause extrinsèque par producteur) · l'error-analysis sur de vrais outputs (le slot `ratings.jsonl` attend ~20 notes Largo)
