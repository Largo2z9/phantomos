...
Un seul point d'entrée pour tout ton travail. Premier étage câblé aujourd'hui · ton acquisition DTC. Le reste s'encode à la demande, le système se construit ses capacités au fur et à mesure que tu encodes.

Ce que le workspace couvre aujourd'hui, ce qui arrive ensuite, ce qui s'active quand tu branches. Pas une liste de features, une carte de ton travail.
...
- **Atlas brand · cartographie holistique data e-commerce** v2.36+. Concept canonique upstream qui structure la matière brand (audiences + products + angles + creatives + scoring + verbatims + tests). Navigable via `/phantom` cockpit. Skills d'enrichissement progressif P0 → P5 (snapshot-brand · define-specs · profile-audience · produce-paid-angles · score-matrix · compose-creative · etc.). Chaîne créa cadrage → gate : frame-regime cadre le régime · produce-paid-angles génère et score les angles · evaluate-concept gate les concepts · weave-hooks tisse le paquet de scripts · compose-creative produit · qc-creative gate le rendu.
...
La chaîne passe trois gates avant diffusion : un gate **stratégie** (le régime de production est cadré et validé en amont), un gate **concept** (chaque concept candidat reçoit un verdict avant le tissage des scripts), un gate **rendu** (le visuel final est contrôlé avant livraison). À la sortie du gate concept, deux routes : **Route A**, production full IA gatée de bout en bout dans le workspace · **Route B**, export d'un brief structuré pour tes créatifs humains, poussé vers l'outil de centralisation détecté dans ton setup (jamais présumé).
...
## Ce qui vit encore dans les outils natifs

- Tendances sociales temps réel (TikTok, Instagram).
- Veille communautaire sociale en direct.
- Montage vidéo et motion design.

Ces blocs vivent dans les outils natifs aujourd'hui (Motion, Foreplay, Triple Whale, CapCut), pas encore câblés dans le workspace. Le pont s'encode à la demande · tu branches le flux (export d'assets, signal de veille, rendu vidéo) et le système l'intègre dans la pipeline quand le kit est connecté.
...
**Cross-brand performance review.** A recurring audit skill reads `learnings.json` across the portfolio, identifies patterns that generalize, and proposes promoting them to `resources/` as shared playbooks. It runs proactively, surfacing what the operator has not yet consolidated rather than waiting to be asked.
...
**Time in the saddle.** The first value is felt from session one · context loaded, no re-briefing. The compound, where encoded decisions reduce cognitive load and speed up delivery, lands between month three and month six of active use for a DTC solo operator, faster for a consultant running five to eight client brands. A pre-traction founder who adopts day one typically encodes hypotheses that pivot within weeks, so the system densifies on a thesis still in flux. That is a question of moment, not a ceiling · once the thesis stabilizes, the same encoding pays off in full. See `fit.md` for adoption timing by profile.
...
See [`fit.md`](fit.md) for a full audit of best fit, conditional fit, and timing by profile (when adopting now pays off versus when waiting a few weeks for the thesis to stabilize serves the operator better).
...
**Scénario 5 · Friction tracking Notion → push.** Une équipe d'opérateurs logge les frictions usage (verbatims clients, retours support) directement dans Notion sous la collection Frictions usage. `--mode=pull` ingère les frictions dans `brands/{slug}/frictions/{FRC-NN}.json` (schema canon v1.0). Skills downstream (produce-paid-angles, score-matrix) consument les frictions pour prioriser les territoires qui adressent les pains réels.
