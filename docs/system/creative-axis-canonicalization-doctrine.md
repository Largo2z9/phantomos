...
> Canonique v2.88.0+. Doctrine canon qui codifie la hiérarchie créative stratégique (brand promise · big idea audience-scoped · axe créatif · créa instance) et le mapping du sous-workflow A Creative Strategy v3 vers l'équation compositionnelle v3.1 NOYAU × CONTEXTE × MODIFIEURS. Doctrine sœur de `creative-formula.md` v3.1 (équation maître substrate · CAC pose le niveau au-dessus stratégique audience-scoped), `creative-mechanics-registry.md` (29+ mécaniques canonisées · CAC référence par axe créatif), `output-clarity-doctrine.md` v2.79.2+ (visualisers matriciel ASCII enforcement), `pre-gate-evaluator-doctrine.md` v2.90.0, annoncée v2.88.1 (evaluator-optimizer A6 5 checks structurels). Ferme le gap *"comment passer d'une requête opérateur DTC à des assets publicitaires cohérents produits depuis l'atlas brand · qualité créative ne se joue pas dans la production IA mais dans la décomposition stratégique amont"* posé en R&D Creative Strategy session 25-27 mai 2026.

> **⚠ SUPERSEDED PARTIEL · v2.89.0 (2026-06-07), RÉCONCILIÉ v2.90.0 (2026-06-11).** Le `creative_axis.schema` et **HR-CAC-4** restent obsolètes (jamais construits · le concept approuvé vit dans `creatives/{batch}/concepts/CPT-NN.json` (verdict embarqué `#evaluation`), pas dans `axes/{id}.json`). EN REVANCHE le gate A6 est désormais RÉEL : `pre-gate-evaluator-doctrine.md` est shippée v2.90.0 et la skill `evaluate-concept` porte les 5 checks (gate CONCEPTS stratégique, orthogonal au gate RENDU `qc-creative` v2.89.0 : stratégie → concept → rendu, trois gates). Le workflow A est opérationnalisé : `frame-regime` (A1-A3), `produce-paid-angles` v1.12 (A4-A5), `evaluate-concept` (A6), `weave-hooks` (A7-A8, produit le genome-package). RESTENT CANON : la hiérarchie créative et le mapping vers l'équation v3.1.
...
- **L'axe créatif cristallise un ANGLE.** L'angle est le **pari stratégique** sélectionné en amont (5 coordonnées · entrée/awareness, moteur, source de croyance, négatif, recadrage · cf `docs/doctrine/angle-strategy-doctrine.md`), jetable et nombreux · l'axe créatif est sa **cristallisation** persistée et typée (mécanique-device + archetype + typology). La sélection d'angle PRODUIT le `angle.formula` qui s'encode dans le CONTEXTE de l'axe · elle remplit l'étage entre big idea et axe créatif, elle ne pose pas d'échelle rivale.
...
| Evaluator-optimizer | (NEW pattern doctrine) | Cf `pre-gate-evaluator-doctrine.md` v2.90.0 (annoncée v2.88.1) |
...
- `pre-gate-evaluator-doctrine.md` v2.90.0 (annoncée v2.88.1) · pattern evaluator-optimizer 5 checks structurels · HR-CAC-4 + HR-CAC-5 enforcement
...
- **First applications** · Sprint v2.88.0 · profile.schema.json v2.2 (big_idea + fears) · creative-mechanics-registry.md (typology_st + temporal_subtype + mapping initial 34 fiches). Sprint v2.88.1 · annonce de pre-gate-evaluator-doctrine.md (ship effectif v2.90.0) + creative_axis.schema.json. Sprint v2.88.2 · compose-creative-batch + compose-creative v1.9 incarnation_only + learn-from-session brief_strategy_intake.
