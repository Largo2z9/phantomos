...
patch_notes:
  - date: "2026-06-14"
    change: >
      ...
      L'identité opérateur (nom, rôle, adresse) est WORKSPACE-level (operator/profile.json,
      transversale aux marques, captée au premier contact et progressivement, jamais
      questionnaire), pas brand-level. Step 0 ne demande PLUS le nom · il ne possède que
      la relation à la marque (brand-specific) et LIT l'identité opérateur. La carte ouverte
      appliquée à l'opérateur (null identity = inconnu à capter avec levier, pas blanc
      silencieux) est portée au workspace · cf open-map-reasoning.md § la carte ouverte vaut
      aussi pour l'opérateur + CLAUDE.md Language & register. Correctif strict.
  - date: "2026-06-14"
    change: >
      Calibration de l'adresse opérateur (D#515 · carte ouverte appliquée à la relation).
      Step 0 · comment l'opérateur veut être appelé est établi au même titre que la
      relation à la marque · déduit si dispo (signature, identity.name d'une session
      passée), sinon demandé une fois légèrement (« je t'appelle comment ? »), persisté
      dans operator/profile.json#identity.name. Un identity.name à null laissé en silence
      = un inconnu non relevé (l'agent flague tout inconnu sur la marque, il doit la même
      honnêteté sur l'opérateur, il ne procède pas anonyme par défaut). Registre tu/vous
      toujours détecté pas demandé. Additif strict.
  - date: "2026-06-14"
    change: >
      Bandeau de position · accompagnement du parcours (D#513). Le récit de handoff
      porte le bandeau de position du parcours UNIFIÉ (porte → close), jamais une
      numérotation locale qui repart à zéro au passage vers build-atlas-complete ·
      l'opérateur ne recolle plus deux moitiés. Posture qui rend et propose, jamais
      un gate de plus. Cf onboarding-setup-flow.md § Câblage principe 5. Additif strict.
  - date: "2026-06-14"
    change: >
      Recon & cadrage du chantier (v2.91). Step 2 réécrit · le scan produit passe en
      DEUX spawns Task de snapshot-brand, jamais inline · spawn #1 recon (Step 1.5 ·
      archétype + héros candidat + plan dimensionné + pré-amorces), l'orchestrateur déplie
      le rapport en clair, gate « valide le chantier » (disclosure existant rendu smart),
      puis spawn #2 deep scan dimensionné. Résout la contradiction « sous-agent muet » vs
      « inférence visible » (l'inférence se fait SUR le rapport, pas en re-dérivant le scan
      inline) qui licenciait le hand-roll maigre observé sur onday.fr. NEW garde-fou
      no-inline-scan dur · l'open-ended final « un angle que le site ne dit pas » remplacé
      par une pré-amorce sourcée. Compte de gates 3 → 2. Doctrine ·
      docs/system/onboarding-setup-flow.md § la recon dimensionne le chantier. Additif/fix,
      aucune logique de délégation retirée.
  - date: "2026-06-14"
    change: >
      Orchestration raccord (BRIQUE 1). onboard-brand ne s'arrête plus à la structure ·
      après son close setup (structure + scan + validate), il handoff explicitement vers
      build-atlas-complete pour les phases 4 à 9 (audiences, voix client, angles, scoring,
      vue d'ensemble, close en posture d'investigation). Délégation, pas réimplémentation ·
      onboard-brand reste la porte, le squelette et le routeur. NEW Step 6 (handoff) inséré
      après Step 5. Scan (Step 2) aligné sur le mode LIVE · inférence visible dans le fil
      principal, seul le mécanique brut part en muet (fin du "runs silently"). Aucune logique
      retirée · ajouts additifs stricts. Doctrine référencée ·
      docs/system/onboarding-setup-flow.md (câblage enrich pas create, phases 0 à 9).
---
...
3. **Passer le contexte, pas le re-construire.** Input · brand slug, profil opérateur, langue/registre depuis `operator/profile.json`. `build-atlas-complete` lit `brands/{slug}/_snapshot.md` et reprend le pipeline à la cartographie des audiences (phase 4). Il ne re-scanne pas, ne re-pose pas la structure · le setup l'a déjà fait.
