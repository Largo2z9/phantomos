...
patch_notes:
  ...
  v1.17.0:
    - "2026-06-14 · D#519 · le garde-fou de profondeur ne vit plus seulement au CLOSE. Cause racine isolée sur le run onday · la postcondition use_cases/spectrum était posée UNIQUEMENT à Step 9, or les runs pausent à un gate opérateur AVANT le close (onday au gate audience, karacare à confirmed_products) · un filet terminal ne tire jamais sur un run gated, donc tout le raisonnement narré dans le fil s'évapore (use_cases vide, spectrum absent, audience = clone octet-pour-octet du template, le vrai raisonnement gelé dans .workflow.json#pending). FIX · checkpoint-sur-ARTEFACT re-posé à CHAQUE frontière de gate · (1) avant de spawn map-audiences, l'orchestrateur LIT spec.json et refuse de scaffolder tant que use_cases non-vide + ≥1 speculative + spectrum.json existe (stage launch) · (2) à l'entrée de GATE INTERMÉDIAIRE 2, re-vérifier que les profils audience écrits ne sont pas le template dégénéré avant de surfacer le gate. Jamais avancer sur la narration, jamais figer la fuite derrière une pause. Filets post-hoc correspondants (D#519) dans validate-all.py · use_cases_missing (le cas VIDE, angle mort de use_cases_no_speculative) + audience_profile_template_clone. Forme canon-correcte · postcondition-sur-artefact, jamais pré-validation du raisonnement (Master rule). Backward compat strict additif. Zéro em-dash."
  v1.16.0:
    - "2026-06-14 · D#518 · réconciliation des dents + le Spectre dirige le fetch. (1) Step 2.5 sous-bloc A/B · les 'dents mécaniques gate write-to-context' annoncées maintainer-only sont remplacées par le verrou RÉEL canon · la postcondition orchestrateur sur l'ARTEFACT (use_cases ≥1 speculative + spectrum.json existe) + le filet post-hoc validate-all (use_cases_no_speculative) et le back-ref angle tracé par validate-resources · PAS un gate de pré-validation au write (Master rule · pré-valider le raisonnement = interdit). Anti-fix-à-moitié · la prose ne promet plus une dent que le canon interdit. (2) NEW · le Spectre dirige le fetch downstream · ses zones blanches (coverage_market unknown/blank) routent watch-competitors (purity rule · l'orchestrateur route, ne fetch pas) et le pont watch-competitors→coverage_market les remplit. Doctrine NEW docs/system/scrape-as-allocation.md. Backward compat strict additif. Zéro em-dash."
  v1.15.0:
    - "2026-06-14 · rebuild logique d'encodage (D#517). Close Step 9 Déduit enrichi · (1) le critique est à DEUX faces, toujours potentiel · opportunités ET faiblesses, en hypothèses-avec-confiance · humble-contextuel (juger au stade, ne jamais challenger un cœur prouvé, chaque faiblesse remise dans son stade + le contexte qui la confirmerait) · les 3 couches claim/réel/opportunité jamais confondues. (2) Proactif par MATÉRIALITÉ jamais par curiosité (le système creuse/demande quand un flou change une conclusion qui paie · valeur d'info = incertitude × poids décision). (3) Postcondition sur l'ARTEFACT jamais sur l'invocation · l'orchestrateur vérifie l'empreinte écrite (use_cases avec spéculatif + spectrum.json existe) pas le fait d'avoir délégué · la précondition teste la qualité, pas l'existence. Backward compat strict additif. Zéro em-dash."
  v1.14.0:
    - "2026-06-14 · profondeur d'exploration + reco proactive (D#514, suite du 2e run onday). (1) Step 2.5 sous-bloc A · use_cases[] doit inclure le tier NON-ÉVIDENT (2-3 adjacent/speculative), un use_cases tout-evident = exploration avortée (observé · 4 use_cases zéro speculative → audiences non servies jamais sorties · garde-fou de fond dans map-audiences F1 v1.4.1). (2) Sous-bloc B · spectre_mode=auto durci · la carte RUNS, spectrum.json écrit, pas optionnel · launch sans spectrum.json = sous-bloc B sauté, invalide (observé · spectrum.json absent malgré stage launch, le négatif lu en prose mais jamais cartographié). (3) Bandeau · le prochain move est une RECOMMANDATION raisonnée dérivée de l'état de la carte (ce qui bloque / débloque / définit la catégorie), jamais un 'avancer ou creuser ?' symétrique nu · réinvoque le no-orphan-output de contextual-intelligence à chaque handoff (observé · close symétrique qui reporte la réflexion sur l'opérateur). Backward compat strict additif. Zéro em-dash (middle dot)."
  v1.13.0:
    - "2026-06-14 · accompagnement du parcours · bandeau de position (D#513). NEW section « Bandeau de position » · à chaque sortie de phase, le rendu PRÉFIXE le no-orphan-output (les deux gestes) par une ligne de position (phase courante + sa place dans le parcours, ✓ faites / ◐ restantes nommées) + une phrase nommant la pièce qui vient de s'allumer. La position se LIT sur la structure des paliers + la Matrix existantes (miroir des 10 phases de onboarding-setup-flow.md), zéro nouvel état · session-state porte déjà step_current/step_total, on le rend en live. Parcours UNIFIÉ à travers la couture onboard-brand→build-atlas (le compteur ne repart pas à zéro). POSTURE jamais gate (rend et propose, ne demande pas la permission · respecte gates-au-macro-seulement, les deux gestes restent des affordances). 2 AP bannis · barre N/M graphique rigide (mentirait dès qu'une phase se reporte · compteur qualitatif zones-nettes/brouillard-avec-levier), doctrine sœur ou viewer re-codé (vit dans le fil, pas une commande séparée, dupliquerait /phantom). ~90% câblage existant (no-orphan-output + deux gestes + atlas vivant + todo persistée), ~10% neuf (le gabarit 3 lignes + la lecture de position à chaque handoff). Enrich pas create. Backward compat strict additif. Zéro em-dash (middle dot)."
  v1.12.0:
    - "2026-06-14 · intelligence contextuelle experte câblée au runtime (D#512). NIVEAU LIVE macro réordonné · ouvre par la lecture de l'échiquier (position / sophistication + négatif concurrentiel) AVANT de dériver les audiences · ordre canon position → négatif → audiences-du-mécanisme → priorité-éco → verdict (la chaîne experte · docs/doctrine/strategic-diagnostic-doctrine.md). Step 2.5 sous-bloc A · spec.use_cases[] peuplé posé en PRÉCONDITION DURE de la Phase 3 (sans le pont mécanisme→usage, la dérivation retombe sur le miroir des avis · biais du survivant, audiences-cartography Pitfall 8). Step 3 · précondition dure ajoutée (ne pas dériver les audiences tant que use_cases[] vide). Close · tranche le verdict de positionnement quand les lectures convergent (vs question ouverte). Frontmatter · ref strategic-diagnostic-doctrine.md re-qualifiée vers docs/doctrine/. Prose-tightening (trust the model) · la dent mécanique correspondante = gate write-to-context sur l'écriture des audiences, maintainer-only, posée séparément [SUPERSEDED par D#518 · le gate write-to-context était une pré-validation interdite par le Master rule · remplacé par la postcondition-sur-artefact + le post-hoc validate-all · cf entrée v1.16.0]. Backward compat strict additif. Zéro em-dash (middle dot)."
  v1.11.0:
    - "2026-06-14 · recon & cadrage du chantier (touch léger orchestrateur). snapshot-brand rend désormais son rapport de recon (NEW Step 1.5) AVANT le deep scan · archétype de cartographie (mono-héros ≤~8 / catalogue resserré ~8-40 / gros catalogue >~40 / edge marketplace) dimensionné sur products_detected (classifieur Step 2bis appelé en avance de phase), héros candidat, plan de scan + ETA, 2-4 pré-amorces inconnus-typés. L'archétype alimente l'hypothèse de territoires candidats au NIVEAU LIVE macro (Step 2) et dimensionne la profondeur du scan. Gate Intermédiaire 1 inchangé · il valide le territoire produit sur la carte RICHE post-scan (décompo NIVEAU 1-4), la recon étant le cadrage amont (prose, jamais matrice · respecte zéro-tableau-ASCII-en-LIVE). En pilotage orchestré, snapshot tourne en deux temps (recon puis deep scan dimensionné · cf onboard-brand Step 2 + snapshot Step 1.5). Touch léger · délégation et chain 4 paliers inchangés, --fast-track préservé. Opérationnalise open-map-reasoning D#508 + câblage 5-axes D#509 à la porte d'entrée. Backward compat strict additif. Zéro em-dash (séparateur middle dot)."
  v1.10.0:
    - "2026-06-14 · scraping marché/compétitif reconnecté · le refactor progressive cartography v1.4.0 avait remplacé l'ancien Step 3 `deepen-brand-context (mine-voc + mine-vom)` par `mine-voc + profile-audience` seuls, DÉCROCHANT `mine-vom` (voix marché) du chaînage, alors que la doctrine `onboarding-setup-flow.md` continue de promettre « voix client ET marché » (phase 5). `watch-competitors` et `trendtrack-enrich-brand` n'avaient jamais été chaînés (standalone dormants). NEW section `Câblage des cinq axes de la récolte` insérée après la section `Doctrine onboarding · 4 principes orchestrateur` et avant Cross-references · elle instrumente les 5 axes de décompo e-com de la doctrine onboarding (économie unitaire · funnel complet · média · créa/message · marché/compétitif). RECONNEXIONS concrètes · (1) `mine-vom` RESTAURÉ comme Sub-step 4c brand-wide offert (axe marché · alimente brand.json#/market + profile voice.market_vernacular + le substrat que le Spectre Step 2.5 et les angles consomment) · règle « voix client ET marché » de nouveau vraie. (2) `watch-competitors` + `trendtrack-enrich-brand` wirés comme palier marché/compétitif OFFERT (axe marché/compétitif · pré-mine candidat en fond ou offert au Gate A / close · Meta Ads Library + intel brand externe). (3) Axe média offert quand le compte est branché · `audit-meta-account` + `audit-google-pmax` + `analyze-perf`, levier `connect-source` sur le compte publicitaire · compte non branché = structure de compte + budget + ROAS réel posés en inconnu typé avec levier. (4) Axe économie unitaire (CAC/LTV/marge/payback/repeat) + axe funnel (checkout/email/rétention/LTV) = pas-encore-outillés honnêtes · aucun skill ne les calcule depuis un scan · posés en inconnu typé via `pending-validations.md`, levier `connect-source` sur commerce + analytique puis `analyze-perf`, plus capture opérateur pour la marge brute · surfacés au close phase 9, jamais inventés en cours de route. Le close Step 9 lit désormais la carte sur les CINQ axes (axes outillés rendus, axes non branchés en brouillard avec levier pré-amorcé, écrits dans la structure). Règles dures HR-AXIS-1 à HR-AXIS-6 + AP-AXIS-1 à AP-AXIS-5 ajoutées. Matrix Step 4c + palier marché/compétitif ajoutés à la table. Cross-references enrichies (mine-vom, watch-competitors, trendtrack-enrich-brand, audit-meta-account, audit-google-pmax, analyze-perf, connect-source, study-niche-marketdeepdive, decompose-ad, produce-positioning-canvas). Application directe de `open-map-reasoning.md` (raisonnement à carte ouverte) sur la grille e-commerce complète. Backward compat strict additif · zéro logique existante retirée · Steps 0-10 + gates + Phase Atlas Visibility + NIVEAU LIVE + 4 principes onboarding tous préservés · `mine-vom` restauré en Sub-step OFFERT (jamais bloquant, n'altère pas le chain --fast-track). Zéro em-dash (séparateur middle dot)."
  v1.9.0:
    - "2026-06-14 · onboarding doctrine 4 principes instrumentés · NEW section `Doctrine onboarding · 4 principes orchestrateur (canon onboarding-setup-flow)` insérée après Guardrails et avant Cross-references. Référence la doctrine racine `docs/system/onboarding-setup-flow.md` plutôt que de la rédupliquer. Instrumente · (0) PORTE en amont · build-atlas-complete accepte d'être précédé par le pré-vol/inférence profil/pacte d'accueil de onboard-brand comme phase 0 (délégation, pas duplication · onboard-brand handoff explicite vers ce pipeline pour les phases 4 à 9). (1) DEUX GESTES par pièce · à chaque sortie de phase (audience, angle, territoire) remonter au niveau orchestrateur le doublet agir/creuser que les producteurs portent déjà localement, comme affordance permanente, pas seulement au close. (2) INCONNUS = cases ouvertes navigables avec levier in situ · chaque inconnu (zone blanche d'angles, trou de scoring, draft à confiance faible) devient une case avec son levier pré-amorcé écrite dans pending-validations au moment où elle surgit, pas une liste agrégée en clôture. (3) EXHAUSTIVITÉ offerte + reportable · à chaque cap (top-3 audiences, top-5 angles, top-3 axes) présenter `tout cartographier maintenant ou je reporte le reste dans la todo`, expliquer ce qu'on perd à sauter, écrire les poches non couvertes comme items reprenables vers pending-validations avec leur levier. PLUS · scan en direct inférence visible (jamais sous-agent muet) et mine-voc + profile-audience pré-minés en fond pendant l'atelier d'identité. Règles dures ajoutées (HR-ONB-1 à HR-ONB-4 + AP-ONB-1 à AP-ONB-4). Backward compat strict additif · zéro logique existante retirée · Steps 0-10 + gates intermédiaires + Gate A + Gate B + Phase Atlas Visibility + NIVEAU LIVE + disclosure NIVEAU 0 tous préservés · section + hooks ajoutés seulement. Zéro em-dash (séparateur middle dot)."
  v1.7.1:
