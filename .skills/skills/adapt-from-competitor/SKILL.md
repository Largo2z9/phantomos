...
patch_notes_brick4_step_e:
  - "2.0.0 (Brique 4 étape E · repath LECTURE decompo source RCV + ÉCRITURE variante CRT batch-du-jour + split genome/creative/sidecar + mkdir-claim + variant_of cross-namespace RCV) · La forme plate `competitive-intel/decomposed/{CRT-NN}.json` (source concurrente lue) et `produced/{CRT-M}.json` (variante écrite) sont ABANDONNÉES, déjà rejetées runtime par le gate write-to-context (ALLOWED_PATH_PATTERNS briques 1+3). adapt-from-competitor est le 3e ÉCRIVAIN de la maison créa (après compose étape B, recompose étape C), DERNIER maillon de la chaîne. (1) REPATH LECTURE · adapt lit la décompo SOURCE concurrente sous `brands/{slug}/competitive-intel/{batch}/{RCV-NN}/decomposition.json` (namespace RCV, schéma natif reverse `decomposition/1.0` produit par decompose-ad v2.3.0 bras REVERSE), PAS la forme flat-legacy `competitive-intel/decomposed/{CRT-NN}.json` (mauvais namespace CRT + mauvais nom de fichier creative.json sub-couche). L'id source RCV-NN est GLOBAL au namespace RCV (batch-indépendant) → le lookup SCANNE TOUS les dossiers `competitive-intel/*/{batch}/` pour localiser le `{RCV-NN}`. (2) REPATH ÉCRITURE · adapt PRODUIT une variante = un NOUVEAU `CRT-NN` dans le batch DU JOUR (la variante va dans le run courant par cohérence-de-run ; le RCV source est trouvé par scan cross-batch, la variante n'hérite PAS de son batch). mkdir-CLAIM STEP 0 atomique (résoudre {batch} du jour, scanner max CRT cross-batch SÉPARÉMENT du max RCV, `mkdir` atomique du nouveau `{CRT-NN}/` + `produced/`, retry +1 sur EEXIST, refus-AVANT-claim). SPLIT persist · genome.json (ADN adapté brand-side · genome/1.2) + creative.json (lignage · variant_of=RCV-source · creative/1.4) + produced/{slug}.json (sidecar manifest binaire · produced-asset/1.0) + brief.md. Le binaire .jpg écrit DIRECT sous produced/, le sidecar json passe par le gate. Miroir compose-creative v1.9.0 + recompose-creative v1.4.0 (mkdir-claim + split). Matérialisation de la règle canon HR-AC-CANON-1 (était phantom). (3) variant_of CROSS-NAMESPACE · variant_of = le `RCV-NN` source (pattern `^(CRT|RCV)-[0-9]{2,4}$` ; pour adapt c'est RCV, le lignage cross-namespace concurrent→nous). Vit dans `creative.json#lineage.variant_of`. C'est LE cas d'usage qui justifie que variant_of accepte RCV (decompose=CRT interne, recompose=CRT, adapt=RCV). Drop du suffixe bricolage '-competitor' (le namespace RCV porte déjà la sémantique concurrent). (4) variant_axis SINGLE · adapt est multi-axe CONCEPTUELLEMENT (audience + pain + proof + mécanisme + offre simultanément) MAIS persiste `variant_axis` SINGLE (l'axe dominant) OU null, conforme à l'enum canonique creative.schema `[photo_swap, promo_toggle, hook_swap, background_swap, persona_split, null]` (jamais un array · casserait le contrat single-enum partagé par recompose). Si le lignage multi-axe doit être capturé, il vit dans un champ creative.json séparé `lineage.adaptation_axes[]`, PAS dans variant_axis. Suppression de tout vocab variant_axis hors enum canonique. (5) VERSION SKEW résolu · `_schema_version` aligné par fichier écrit · `creative/1.4` (lignage) + `genome/1.2` (ADN) pour la variante écrite ; adapt LIT `decomposition/1.0` en source. Cross-ref `resources/conventions/creative-storage.md` (D#481) + sibling compose-creative v1.9.0 + recompose-creative v1.4.0. Backward compat · forme cible imposée par le gate write-to-context.py · les anciennes écritures plates étaient déjà rejetées runtime, ce patch aligne le SOP sur le gate. Régénérer `_manifest.json` (build-manifest.py) après cet édit · disambiguates_against change. NOTE Notion bridge auto-wire DEFERRED v2.88.0+ (sync-creatives-to-notion pas shipped)."
description: >
