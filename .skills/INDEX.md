...
Last update : 2026-06-10 (v2.89.2 coherence pass). The per-skill cards below cover the 46-skill core mapped at v2.33 ; the exhaustive and always-current list of shipped skills (82) lives in `_manifest.json` (regenerated by `build-manifest.py`) and in `.skills/README.md`. A skill absent from the cards is not absent from the product.
...
**P3 · Matrix + scoring**
  weight-dimensions (compute coefficients) vers score-matrix
...
| Domain | Phase | Prerequisites | Next steps |
|---|---|---|---|
| product | P3 (modulator) | profile-audience populated · angles disponibles | score-matrix |
